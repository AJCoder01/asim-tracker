...
* **Data Ingestion & Processing (`data_pipeline/`):**
  ...
  * [`data_pipeline/scrapers/redis_queue.py`](data_pipeline/scrapers/redis_queue.py): Redis connection pool manager buffering live streams.
  * [`data_pipeline/processors/text_processor.py`](data_pipeline/processors/text_processor.py): Quantized ONNX FinBERT model handling sentiment score mapping.
  * [`data_pipeline/processors/market_processor.py`](data_pipeline/processors/market_processor.py): Level-3 DWT filter configurations and OBI calculations.
* **Quantitative Modeling (`model_engine/`):**
  * [`model_engine/networks.py`](model_engine/networks.py): Bilinear cross-attention module in PyTorch.
  * [`model_engine/optimization.py`](model_engine/optimization.py): Hawkes process MLE parameter estimator.
  * [`model_engine/loss.py`](model_engine/loss.py): Custom target losses.
* **Execution Sentinel (`execution_system/`):**
  * [`execution_system/broker_api.py`](execution_system/broker_api.py): Handles TOTP 2FA daily sessions and resets.
  * [`execution_system/order_routing.py`](execution_system/order_routing.py): Under 10 OPS pegged limit order transmitter.
  * [`execution_system/sentinel.py`](execution_system/sentinel.py): Circuit breakers and stage 2+ ASM/GSM asset blacklist watchdogs.
* **Backtester Loop (`backtester/`):**
  * [`backtester/event_driven.py`](backtester/event_driven.py): NumPy chronological event-driven simulator.